...
import fr.afgj.coinbot.service.impl.CurrencyServiceImpl;

public class ParsageJson {
	...
	public void getMarket() {
			...
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					if (jsonObj.isNull("24h_volume_eur")) {
						...
						market.setVolume_eur24h(jsonObj.getDouble("24h_volume_eur"));
					}
		...
		for (Market m : listMarket) {
			...
			currency.setName(m.getName());
			currency.setSymbol(m.getSymbol());
			currency.setRank( m.getRank() );
			currency.setVolumeeur( m.getVolume_eur24h() );
			currency.setVolumeusd( m.getVolume_usd24h() );
			System.out.println("Saving ....");
			System.out.println(cs.saveCurrency(currency).toString());
